...
$restimpo_themever = "1.4.2";					//Theme version
 ...
 * 
 */
function sw_title_func( $atts, $content ){
    $titleb = '<h2 class="entry-headline"><span class="entry-headline-text">';
    $titlee = '</span></h2>';
    $return = $titleb . $content . $titlee;
    	return $return;
}
add_shortcode( 'sw_title', 'sw_title_func' );

function sw_text_block_func( $atts, $content){
    
    switch ($atts['allign']){
      case 'left':
          $blockb = '<div class="sw-text-block-left"><p>';
          break;
      case 'right':
          $blockb = '<div class="sw-text-block-right"><p>';
          break;
    };
    
    $blocke = '</p></div>';
    
    $return = $blockb . $content . $blocke;
    
    return $return;
}
add_shortcode('sw_text_block', 'sw_text_block_func')
?>
